...
data = pd.read_csv("DistanceGluedWithout2.csv")
...
dump(gs, 'TrainedModels/gsPlusWithout.joblib', protocol=2)

clf = tree.DecisionTreeClassifier(criterion=gs.best_params_["criterion"], max_depth=gs.best_params_["max_depth"])

clf = clf.fit(xtrain, ytrain)
dump(clf, 'TrainedModels/clfMatrixPlusWithout.joblib', protocol=2)
